//these variables correspond to the HTML elements on the page.
...
//gets any previous search queries from the local storage and displays them on the page.
var citiesSearched = JSON.parse(window.localStorage.getItem("myCities")) || [];
//API key
var weatherKey = import.meta.env.VITE_API_KEY
//formatting date as month, day, year.
var timeNow = moment().format('[(]MM[/]DD[/]YYYY[)]');

//this function calls the openWeather API using the user's search query as a parameter
function getWeather(cityName){

    //this structures the URL for the API call 
    var requestURL = `https://api.openweathermap.org/data/2.5/weather?q=${cityName}&units=imperial&appid=${weatherKey}`;

        //the fect request will return the data as a JSON object
        fetch(requestURL)
...
//This function takes the date retunred from the API data object and returns it as a string which is formatted.
